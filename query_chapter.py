import json, hashlib, math, os, re, sys
...
def main():
	...
	max_result = 10
	if len(sys.argv) < 2:
		max_result = int(sys.argv[2])
	result = return_bm25_scores(query)[:max_result]
	print(f"\nThe following is the list of top {max_result} results for your query ranging from the best match to the worst match.\n")
	print(f"\n{"No.":<5} {"Book":<16} {"Chapter":<8} Search Score")
	for index, tlist in enumerate(result):
		no = str(index + 1) + "."
		print("{:<5} {:<16} {:<8} {}".format(no, tlist[0].split("_")[0], tlist[0].split("_")[2][:1], str(tlist[1])))
	print("\n")
